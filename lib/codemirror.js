...
function onTouchStart(e) {
  ...
  function onTouchMove (e) {
    ...
    var from = sel.ranges[0].head;
    var to = sel.ranges[0].anchor;
    if (touchMode == "single" && cm.options.dragDrop && dragAndDrop && !isReadOnly(cm) && cmp(from, to) && cmp(pos,from)>0 && cmp(to, pos)>0 && lastClick.time < now-100) {
      touchMode = "drag";     
    }
    else
     if (touchMode == "single" && lastClick.time < now-100) {
  ...
  function onTouchEnd(e) {
    ...
    e_preventDefault(e);
      ...
      if (cmp(pos, from) > 0  || cmp(to, pos) > 0) {
        cm.state.draggingText(e);
      }
      else {
        var text = cm.getSelection();
        var curFrom = cm.doc.sel.ranges[0].head, curTo = cm.doc.sel.ranges[0].anchor;
